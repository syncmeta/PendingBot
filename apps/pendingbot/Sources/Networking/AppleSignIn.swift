...
enum AppleSignIn {
    ...
    /// Supabase. The returned session is also automatically persisted by
    /// the supabase-swift Auth client and surfaced via its authStateChanges.
    static func run() async throws -> Session {
        ...
        let credential: ASAuthorizationAppleIDCredential = try await withCheckedThrowingContinuation { cont in
            ...
            objc_setAssociatedObject(controller, &AppleSignInDelegate.holderKey, delegate, .OBJC_ASSOCIATION_RETAIN)
        ...
        let session = try await SupabaseStack.shared.auth.signInWithIdToken(
...
private final class AppleSignInDelegate: NSObject, ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
    ...
    nonisolated func authorizationController(controller: ASAuthorizationController,
        ...
        if let cred = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            continuation.resume(throwing: AppleSignIn.Error.missingIdentityToken)
        }
    }
    ...
    nonisolated func authorizationController(controller: ASAuthorizationController,
        ...
        if let asError = error as? ASAuthorizationError, asError.code == .canceled {
            continuation.resume(throwing: AppleSignIn.Error.userCancelled)
        } else {
            continuation.resume(throwing: AppleSignIn.Error.underlying(error))
        }
    }
...
private func sha256Compute(_ message: [UInt8], into out: inout [UInt8]) {
    let k: [UInt32] = [
