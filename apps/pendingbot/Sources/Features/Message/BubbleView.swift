...
struct BubbleView<Menu: View>: View {
    ...
    var body: some View {
        ...
        // 文本选中只给指针底下这一条 —— macOS 上常开会给每段文字挂一个真
        // NSTextField 并逐帧全量刷新。见 BubbleTextSelection.swift。
        .bubbleTextSelection(id: message.id)
    }
        ...
        HStack(alignment: .top, spacing: 0) {
            ...
            Group {
                if let g = groupSender {
                    // Group conv — sender resolved per-message.
                    switch g.kind {
                    case .bot:
                        BotAvatar(emojiSeed: g.id, colorSeed: g.id, size: 30)
                    case .user:
...
struct AttachmentGrid: View {
            ...
            ForEach(imageRows.indices, id: \.self) { row in
                HStack(spacing: 6) {
                    ForEach(imageRows[row]) { att in
                        // 按格子尺寸降采样解码 —— 110pt 的缩略图不需要 4000px 原图,
                        // ×3 给 retina + 放大留余量。看大图那条路(ImageViewer)不传,
                        // 仍是原尺寸。
    ...
    private func saveToAlbum(_ att: Attachment) {
        Task {
            guard let data = try? await APIClient().download("v1/uploads/\(att.id)"),
...
struct FileAttachmentChip: View {
    ...
    private func openFile() async {
        ...
        do {
            let data = try await APIClient().download("v1/uploads/\(attachment.id)")
