-- 接合 v2 block 3 — RPC crew_add_member_for_subject(成员邀请走 device grant)。
--
-- 背景:relay crew(edge 信箱)建好后,Mac 经 device grant 把 PendingBot 侧
-- 的 bot / 人类账号拉进 crew conversation,让 bot 经现有 group-turn 管线
-- 参与、人类在 iOS 看到群。HTTP 入口 POST /v1/crews/:crewId/members
-- (routes/crew-comms.ts),JWT 与 device grant 双轨。
--
-- 信任模型沿用 20260529014827_crew_rpcs_accept_actor:
--   v_caller := COALESCE(auth.uid(), p_actor_user_id)
-- authenticated(JWT/userClient)调用时 auth.uid() 非空,p_actor_user_id 被
-- 忽略 → 无法冒充;只有 service_role(edge 内,actor 来自已验证 grant 的
-- grantedByUserId)才落到 p_actor_user_id。
--
-- 校验:
--   * crew 存在(temporary_group_meta, temporary_kind='crew')→ 否则 P0002
--   * actor 能为 responsible_subject 出面(_crew_caller_can_act_for_subject)
--   * registered_bot:bot 须 actor 可用 —— 自己的 bot,或非 private 且
--     actor 已通过 user_bot_contacts 加为联系人(对齐 group_invite_bot 思路,
--     但 contacts 即代表"可用",invite 门已在加联系人时过)
--   * human:目标须是 actor 好友(user_contacts)或 subject 成员
--     (user_account 的 user 本人 / group_subject_members 成员)
--
-- 落两张表:temporary_group_members(role='member')+
-- conversation_participants(普通消息流权限,镜像 create_crew_with_captain)。
-- 幂等:已是 active 成员 → 返回现有行(already_member=true),不报错。

BEGIN;

SET search_path TO pendingbot, public;

CREATE OR REPLACE FUNCTION pendingbot.crew_add_member_for_subject(
      ...
      OR EXISTS (
        SELECT 1 FROM pendingbot.subjects s
         WHERE s.id = v_subject_id
           AND s.subject_type = 'user_account'
...
ALTER FUNCTION pendingbot.crew_add_member_for_subject(uuid, uuid, text, uuid, uuid) OWNER TO postgres;
REVOKE ALL ON FUNCTION pendingbot.crew_add_member_for_subject(uuid, uuid, text, uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION pendingbot.crew_add_member_for_subject(uuid, uuid, text, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION pendingbot.crew_add_member_for_subject(uuid, uuid, text, uuid, uuid) TO service_role;
